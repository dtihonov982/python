...
class Path2:
    ...
    def step_on(self, x, y):
        ...
        if self.len() > 1:
            ...
            tk_id = canvas.create_line(p1[0],
                                       p1[1],
                                       x,
                                       y,
                                       fill=self.color)           
            self.list.append((p1[0], p1[0], x, y, tk_id))
...
if __name__ == '__main__':
    ...
    #Орбита эллиптическая
    b3 = [
        Body(xc, yc, 0.0, 0.0, 100.0*(200.0**2), 20.0),
        Body(xc-100.0, yc, 0.0, -200.0, 0.0, 5.0) #,
    ...
    ex = Example(root, b3)
    root.geometry("{}x{}+300+300".format(int(W), int(H)))
    root.mainloop()  
